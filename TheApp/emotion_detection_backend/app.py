...
def extract_features(audio, sr=22050):
    ...
    chroma_stft = librosa.feature.chroma_stft(y=audio, sr=sr).mean(axis=1)
    mfcc = librosa.feature.mfcc(y=audio, sr=sr, n_mfcc=40).mean(axis=1)
    rms = librosa.feature.rms(y=audio).mean()
    mel_spec = librosa.feature.melspectrogram(y=audio, sr=sr).mean(axis=1)
    features = np.hstack([zcr, chroma_stft, mfcc, rms, mel_spec])
    return features

# Function to preprocess the audio
# Function to preprocess the audio
def preprocess_audio(audio, fs=22050):
    # Extract features the same way as during the training
    zcr = librosa.feature.zero_crossing_rate(audio).mean()
    chroma_stft = librosa.feature.chroma_stft(y=audio, sr=fs).mean(axis=1)
    mfcc = librosa.feature.mfcc(y=audio, sr=fs, n_mfcc=40).mean(axis=1)
    rms = librosa.feature.rms(y=audio).mean()
    mel_spec = librosa.feature.melspectrogram(y=audio, sr=fs).mean(axis=1)
    features = np.hstack([zcr, chroma_stft, mfcc, rms, mel_spec])
...
def audio_emotion():
    ...
    try:
        ...
        audio, sr = librosa.load(audio_path, sr=22050)
        # Preprocess the audio to get the feature set
        preprocessed_audio = preprocess_audio(audio, sr)
        # Predict the emotion using the preprocessed audio features
        predictions = audio_model.predict(preprocessed_audio)
        max_index = np.argmax(predictions[0])
        detected_emotion = audio_emotions[max_index]
